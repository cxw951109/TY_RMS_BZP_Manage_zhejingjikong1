...
# 获取单个试剂基本信息及流转记录
@require_http_methods(['GET'])
def homeDrugDetail(request):
    if request.method == 'GET':
        drug_id = request.GET.get("drug_id", "")
        BarCode = request.GET.get('BarCode', '')
        drug_obj = BllMedicament().findEntity(drug_id)
        return render(request, 'home/homeDrugDetail.html', locals())
        ...
        if not varietyId:
            try:
                ...
                if not BarCode:
                    ...
                    MedicamentRecord_obj_list = BllMedicamentRecord().execute(SQL,
                                                                              {'recordType': recordType}).fetchall()
                    ...
                    #
                    # print("**********" * 100)
                    # print(Utils.resultAlchemyData(Utils.mysqlTable2Model(MedicamentRecord_obj_list)))
                    return JsonResponse(
                        {'data': json.loads(Utils.resultAlchemyData(Utils.mysqlTable2Model(MedicamentRecord_obj_list))),
                         "code": 0, "count": drug_obj_list_len, "status": 1})
            ...
            try:
                ...
                drug_obj_list_len = int(Utils.mysqlTable2Model(drug_obj_list_len)[0]["COUNT(*)"])
                return JsonResponse(
                    {'data': json.loads(Utils.resultAlchemyData(Utils.mysqlTable2Model(typeRecord_obj_list))),
                     "code": 0, "count": drug_obj_list_len, "status": 1})
            except Exception as e:
                logger.info('数据异常')
                return JsonResponse(Utils.resultData(0, "数据异常"))
    ...
    if request.method == 'GET':
        ...
        if warningType:
            try:
                ...
                return JsonResponse(
                    {'data': json.loads(Utils.resultAlchemyData(Utils.mysqlTable2Model(warning_obj_list)))})
            except Exception as e:
                logger.info('数据异常')
                return JsonResponse({'data': ''})
...
def homeDescriptionJson(request):
    if request.method == 'GET':
        ...
        try:
            ...
            SQL = SQL_all + sql1
            print('sql 语句是', SQL)
            desc_obj_list = BllWarning().execute(SQL, {'CreateUserName': CreateUserName, 'startDate': startDate,
...
def homeMedicamentDescriptionJson(request):
    if request.method == 'GET':
        ...
        try:
            ...
            SQL = SQL_a + sql1
            desc_obj_list = BllWarning().execute(SQL, {'medicamentName': medicamentName, 'startDate': startDate,
                                                       'endDate': endDate, }).fetchall()
            return JsonResponse({'data': json.loads(Utils.resultAlchemyData(Utils.mysqlTable2Model(desc_obj_list)))})
        except Exception as e:
            logger.info('数据异常')
